...
Item {
    ...
    Flickable {
        ...
        Column {
            ...
            Rectangle {
                ...
                Column {
                    ...
                    Row {
                        ...
                        Repeater {
                            model: [
                                { icon: "🏅", v: page.me ? page.me.completed_count : 0, l: "完成代跑" },
                                { icon: "😴", v: page.me ? page.me.no_show_count : 0, l: "接单爽约" },
                                { icon: "⏰", v: page.me ? page.me.no_show_count_poster : 0, l: "挂单爽约" }
                            ]
                            delegate: Rectangle {
            ...
            MenuCard {
                items: [
                    { icon: "📤", label: "挂单记录", page: "HistoryPage.qml", role: "poster" },
                    { icon: "📥", label: "接单记录", page: "HistoryPage.qml", role: "receiver" },
                    { icon: "📢", label: "公告中心", page: "NoticesPage.qml" },
                    { icon: "📖", label: "使用指南", page: "GuidePage.qml" },
            ...
            MenuCard {
                items: [
                    { icon: "ℹ️", label: "关于乐乐代跑", action: "about" },
    ...
    function onMenu(m) {
        if (m.action === "logout") {
            ...
            Ui.confirm({ title: "乐乐代跑 v1.0.0", text: "校园代跑互助平台\n· 无密码注册，仅邮箱验证码\n· 挂单/接单互助，费用线下当面结算\n· 交易问题请走订单内申诉/举报流程", okText: "知道了" }, function () {})
        } else if (m.page) {
    ...
    function uploadPhoto(fileUrl) {
        ...
        Api.request("/api/auth/me/avatar", { method: "POST", form: fd }).then(function () {
            Ui.loading(false)
            Ui.toast("头像已更新")
            load()
        }).catch(function (e) {
