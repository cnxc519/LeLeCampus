...
function go(view) {
  ...
  ({ dashboard: vDashboard, schools: vSchools, playgrounds: vPlaygrounds, settings: vSettings, notices: vNotices, users: vUsers, orders: vOrders, reports: vReports, books: vBooks, bookreports: vBookReports, version: vVersion })[view]();
}

// 侧栏角标（待处理举报数）
function setBadges(openReports, openBookReports) {
  const rb = $('#report-badge'), bb = $('#bookreport-badge');
  rb.textContent = openReports; rb.classList.toggle('hidden', !openReports);
  bb.textContent = openBookReports; bb.classList.toggle('hidden', !openBookReports);
}
function refreshBadges() {
  api('GET', '/dashboard').then((d) => setBadges(d.open_reports, d.open_book_reports)).catch(() => {});
}

const RUN_ST = { requested: ['待确认', 'orange'], confirmed: ['进行中', 'blue'], completed: ['已完成', 'green'], poster_noshows: ['挂单方爽约', 'red'], receiver_noshows: ['接单方爽约', 'red'], rain_cancelled: ['雨天终止', 'blue'], rejected: ['已拒绝', ''] };
const stTag = (s) => { const [t, c] = RUN_ST[s] || [s, '']; return `<span class="tag ${c}">${t}</span>`; };
  ...
  try {
    ...
    render(`
      <div class="stat-grid">
        ...
        <div class="stat"><div class="num">${d.active_orders}</div><div class="lbl">进行中挂单</div></div>
        <div class="stat"><div class="num">${d.runs_today}</div><div class="lbl">今日跑单</div></div>
        <div class="stat"><div class="num">${d.open_reports}</div><div class="lbl">待处理举报</div></div>
      ...
      <div class="card" style="margin-top:16px">
        <h2>规则参数（可在"规则设置"中调整）</h2>
        <table><tr><th>项目</th><th>数值</th></tr>
          <tr><td>接单方同时间点并发上限</td><td>${d.settings.receiver_max_concurrent} 单</td></tr>
          <tr><td>爽约反驳窗口</td><td>${d.settings.appeal_hours} 小时</td></tr>
          <tr><td>雨天终止默认同意时限</td><td>${d.settings.rain_response_hours} 小时</td></tr>
          <tr><td>挂单次数上限</td><td>${d.settings.run_count_max} 次</td></tr>
        </table>
      </div>`);
    setBadges(d.open_reports, d.open_book_reports);
...
async function vSchools() {
  ...
  render(`
    <div class="card">
      <h2>学校管理（新增学校后，其下可添加操场；App 注册时学校只可选择）</h2>
      <div class="toolbar">
        <input id="school-name" placeholder="学校名称，如：武汉大学">
        <button class="primary" onclick="addSchool()">添加学校</button>
      </div>
      <table>
        <tr><th>ID</th><th>学校</th><th>操场数</th></tr>
        ${d.list.map((s) => `<tr><td>${s.id}</td><td>${esc(s.name)}</td><td>${s.pg_count}</td></tr>`).join('')}
      </table>
    </div>`);
}
...
// ---------- 操场管理 ----------
async function vPlaygrounds() {
  const d = await api('GET', '/playgrounds');
  const schools = await api('GET', '/schools').catch(() => null);
  render(`
    <div class="card">
      <h2>操场管理（新增操场会立即出现在 App 挂单页）</h2>
      <div class="toolbar">
        <select id="pg-school">${schools ? schools.list.map((s) => `<option value="${s.id}">${esc(s.name)}</option>`).join('') : ''}</select>
        <input id="pg-name" placeholder="操场名称，如：信部操场">
        <button class="primary" onclick="addPg()">添加</button>
      </div>
      <table>
        <tr><th>ID</th><th>学校</th><th>操场</th><th>排序</th><th>操作</th></tr>
        ${d.list.map((p) => `<tr>
          <td>${p.id}</td><td>${esc(p.school)}</td>
          <td><input id="pg-name-${p.id}" value="${esc(p.name)}" style="width:160px"></td>
          <td><input id="pg-sort-${p.id}" type="number" value="${p.sort}" style="width:70px"></td>
          <td>
            <button class="small" onclick="savePg(${p.id})">保存</button>
            <button class="small danger" onclick="delPg(${p.id})">删除</button>
          </td></tr>`).join('')}
      </table>
    </div>`);
}
async function addPg() {
  const school_id = +$('#pg-school').value, name = $('#pg-name').value.trim();
  if (!name) return toast('请填写操场名称');
  await api('POST', '/playgrounds', { school_id, name });
  toast('已添加'); vPlaygrounds();
}
async function savePg(id) {
  const name = $('#pg-name-' + id).value.trim(), sort = +$('#pg-sort-' + id).value;
  await api('PUT', `/playgrounds/${id}`, { name, sort });
  toast('已保存');
}
async function delPg(id) {
  if (!confirm('确定删除该操场？')) return;
  try { await api('DELETE', `/playgrounds/${id}`); toast('已删除'); vPlaygrounds(); }
  catch (e) { toast(e.message); }
}
...
async function vSettings() {
  ...
  const rows = [
    ['receiver_max_concurrent', '接单方同一时间点并发上限(单)'],
    ['appeal_hours', '爽约反驳窗口(小时)'],
    ['rain_response_hours', '雨天终止默认同意时限(小时)'],
    ['request_expire_hours', '接单申请未处理自动关闭(小时)'],
    ['avatar_max_kb', '头像大小上限(KB)'],
    ['run_count_default', '挂单次数默认值'],
    ['run_count_max', '挂单次数上限'],
  ];
  render(`
    <div class="card">
      <h2>规则设置（流程参数，修改后立即生效）</h2>
      <table>
        <tr><th>项目</th><th>当前值</th><th>新值</th></tr>
        ${rows.map(([k, lbl]) => `<tr>
          <td>${esc(lbl)}</td><td><b>${s[k] ?? 0}</b></td>
          <td><input id="set-${k}" type="number" value="${s[k] ?? 0}" style="width:120px"></td></tr>`).join('')}
      </table>
      <div class="toolbar" style="margin-top:14px"><button class="primary" onclick="saveSettings()">保存全部</button></div>
    </div>`);
}
async function saveSettings() {
  const body = {};
  $$('#main input[id^="set-"]').forEach((i) => { body[i.id.slice(4)] = +i.value; });
  const d = await api('PUT', '/settings', body);
  toast('已保存');
  vSettings();
}
...
async function vUsers() {
  ...
  render(`
    <div class="card">
      ...
      <table>
        <tr><th>ID</th><th>昵称</th><th>邮箱</th><th>性别</th><th>学校</th><th>完成/爽约</th><th>状态</th><th>操作</th></tr>
        ${d.list.map((u) => `<tr>
          <td>${u.id}</td><td>${esc(u.nickname)}</td><td>${esc(u.email)}</td>
          <td>${u.gender === 'male' ? '男' : '女'}</td><td>${esc(u.school)}</td>
          <td>${u.completed_count}/${u.no_show_count}次</td>
  ...
  render(`
    <div class="card">
      ...
      <table>
        ...
        <tr><td>注册时间</td><td>${esc(u.created_at)}</td><td>完成/爽约</td>
          <td>完成 ${u.completed_count} 单，接单爽约 ${u.no_show_count} 次，挂单爽约 ${u.no_show_count_poster} 次</td></tr>
      </table>
    </div>`);
}
...
async function vOrders() {
  ...
  render(`
    <div class="card">
      <h2>挂单列表</h2>
      <table><tr><th>ID</th><th>委托内容</th><th>操场</th><th>金额(每次)</th><th>次数</th><th>剩余</th><th>性别要求</th><th>状态</th><th>发布人</th></tr>
      ${d.list.map((o) => `<tr>
        <td>${o.id}</td><td>${esc(o.remark)}</td><td>${esc(o.playground.name)}</td>
        <td><b>¥${yuan(o.price_cents)}</b></td>
        <td>${o.run_count}</td><td>${o.remaining}</td>
        <td>${o.gender_required === 'none' ? '无' : o.gender_required === 'male' ? '仅男' : '仅女'}</td>
        <td>${o.status === 'active' ? '<span class="tag green">进行中</span>' : o.status === 'finished' ? '<span class="tag">已结束</span>' : '<span class="tag red">已取消</span>'}</td>
        <td>${o.poster ? esc(o.poster.nickname) : '-'}</td></tr>`).join('')}
      </table>
    </div>`);
}
...
async function vReports() {
  ...
  render(`
    <div class="card">
      <h2>代跑举报（辱骂/威胁/违规等；处理即结案）</h2>
      <table>
        <tr><th>ID</th><th>挂单内容</th><th>日期</th><th>举报人</th><th>挂单方</th><th>接单方</th><th>原因</th><th>时间</th><th>状态</th><th>处理说明</th><th>操作</th></tr>
        ${d.list.map((r) => `<tr>
          <td>${r.id}</td><td>${esc(r.remark)}</td><td>${esc(r.date || '-')}</td>
          <td>${esc(r.reporter_name)}</td><td>${esc(r.poster_name || '-')}</td><td>${esc(r.receiver_name || '-')}</td>
          <td>${esc(r.reason)}</td><td>${fmt(r.created_at)}</td>
          <td>${r.status === 'open' ? '<span class="tag red">待处理</span>' : '<span class="tag green">已处理</span>'}</td>
          <td>${esc(r.note || '-')}</td>
          <td>${r.status === 'open' ? '<button class="small primary" onclick="resolveReport(' + r.id + ')">处理</button>' : ''}</td></tr>`).join('')}
      </table>
    </div>`);
}
async function resolveReport(id) {
  const note = prompt('处理说明（必填）：例如 已核实警告/封禁相关用户');
  if (note === null) return;
  if (!note.trim()) return toast('请填写处理说明');
  await api('POST', `/reports/${id}/resolve`, { note: note.trim() });
  toast('已处理'); vReports();
}
  ...
  render(`
    <div class="card">
      ...
      <table>
        ...
        <tr><td>APK 地址</td><td><input id="v-url" value="${esc(s.apk_url || '')}" style="width:400px" placeholder="/files/apk/lele-daipao.apk 或完整外链 https://..."></td></tr>
